...
- Code coverage reporting via Codecov. Added `@vitest/coverage-v8` devDep and a `coverage` block to `vitest.config.ts` (`provider: "v8"`, `reporter: ["text", "lcovonly"]`, output to `coverage/`, scoped to `src/**/*.ts` + `index.ts`). New `.github/workflows/verify.yml` runs on pushes to `master` and on PRs: pnpm install (frozen lockfile) → typecheck → lint → test → coverage (`vitest run --coverage --coverage.reporter=lcovonly`) → upload `coverage/lcov.info` to Codecov via `codecov/codecov-action` (pinned to SHA `fb8b3582c8e4def4969c97caa2f19720cb33a72f`, v7) with `fail_ci_if_error: false`. cf-crawl is public, so the upload is tokenless (`CODECOV_TOKEN` is optional). Added a Codecov badge to `README.md`.
