...
- **Known issue:** the namespace is out of CI/CD minutes, so every pipeline fails as `ci_quota_exceeded` with no runner assigned. `.gitlab-ci.yml` is valid (`glab ci lint`) but has never executed a single step. `.githooks/pre-push` is the only working gate until this is resolved.
