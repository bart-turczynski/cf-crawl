...
- CI ported from GitHub Actions to GitLab CI. `origin` is GitLab, so `.github/workflows/verify.yml` had never run for this remote — nothing gated pushes or merge requests. New `.gitlab-ci.yml` mirrors it as a single fail-fast `verify` job on `node:22`: corepack (pinning pnpm@11.1.1 from `packageManager`) → `pnpm install --frozen-lockfile` → typecheck → lint → format check → test → coverage. Runs on merge request events and on the default branch, with the pnpm store cached on `pnpm-lock.yaml`. Coverage now also emits a `cobertura` reporter alongside `lcovonly`, exposed as a GitLab `coverage_report` artifact for MR diff annotations, plus a job-level `coverage` regex for the badge. Codecov has no tokenless upload on GitLab (that is GitHub-only), so the upload step runs only when `CODECOV_TOKEN` is set and never fails the pipeline. `.github/workflows/verify.yml` is left in place for any GitHub mirror.
- README status badges removed. The Codecov badge pointed at a `codecov.io/gh/` path while the repository is a private project on GitLab, so it could never reflect this remote — tokenless Codecov uploads are GitHub-only, and no upload had ever run. GitLab-native badges were tried in their place, but the namespace is out of CI/CD minutes (`ci_quota_exceeded`), so they would have advertised a failed pipeline. Re-add the GitLab pipeline and coverage badges once minutes are available.
