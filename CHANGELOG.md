...
- `package.json` `keywords`, matching the ten topics now set on both the GitLab project and its GitHub mirror, which also gained the `package.json` description. All three were empty, so neither forge search nor a future npm listing had anything to match on, and turczynski.pl, which builds its Tools list from registry and forge metadata, had no description to show.
- `scripts/install-skill.sh`, replacing the `ln -sf` one-liner the README used to hand out for installing the skill. It exists because the pre-`skill/` install was a real directory holding the symlink, and a plain `ln -s` of the new directory against that lands _inside_ it — creating `~/.claude/skills/cf-crawl/skill`, reporting success, and leaving the dangling `SKILL.md` untouched. The script recognises that shape, removes it, links `skill/` in its place, and verifies `SKILL.md` is readable through the new link. It removes only a shape this repository provably installed: a foreign symlink, or a directory holding anyone else's work, is refused instead. Idempotent.
...
- Root `skill.md` moved to `skill/SKILL.md`, and `~/.claude/skills/cf-crawl` now links that directory instead of standing as a real directory holding a symlink to the single file. Only the linked file was ever served, so a `references/` beside it would not have been found — and the failure is silent, since a missing reference reads to an agent as a reference nobody wrote. The file also had to be renamed on the way, because a skill's entry point is `SKILL.md`, capitalised. `.claude/skills/` is deliberately not the destination: it is gitignored here, which is what made the earlier project-level copy drift out of sync. The `files` allowlist entry becomes `skill`, so the published tarball ships the directory. **This breaks an existing install on `git pull` alone**: the old `SKILL.md` symlink points at a `skill.md` that no longer exists, so the skill stops appearing with no error to explain it. Run `./scripts/install-skill.sh` after pulling.
...
- `repository`, `homepage` and `bugs` fields in `package.json`, all pointing at the GitLab project. The package carried no repository metadata at all, so nothing in the tree or in a future npm listing named the canonical source — and the dormant GitHub mirror at `github.com/bart-turczynski/cf-crawl`, frozen since PR #7 and 26 commits behind, is what a search turns up instead. A collaborator cloned that mirror, synced it faithfully, and reported bugs against a tree predating v4.0.0. `npm publish` also warns on a missing `repository`. Audited the tree for stale GitHub URLs while here: there are none. Every remaining `GitHub` mention is historical narrative in this changelog and two provenance comments in `.gitlab-ci.yml`, all describing the Actions-to-GitLab-CI migration; those are a record of what happened and stay as written.

### Changed

- `.githooks/pre-push` now checks that `pnpm` is on `PATH` before running the verify chain, and exits with the cause named plus the `corepack enable && corepack prepare --activate` remedy. Previously `set -e` aborted the push on a bare `pnpm: command not found` (exit 127) at the first gate, which is hard to tell apart from a genuinely failing typecheck. Deletion-only pushes still short-circuit before the check, so they need no toolchain at all. npm is deliberately not accepted as a fallback: `pnpm-lock.yaml` is the lockfile and CI installs from it with `--frozen-lockfile`, so an npm-resolved tree would not be the one that was verified. Reported by Robert Araczyńska, who hit it on a clone with only npm installed.
- README's Setup section now states the Node >= 22 and pnpm prerequisites and the corepack one-liner, instead of opening on `pnpm install` and assuming pnpm is already present.
...
- CI pipelines are now only created deliberately, to conserve the namespace's Free-tier CI/CD minutes. `.gitlab-ci.yml` gained a `workflow:` rules block that allows a pipeline only for a version tag matching `v<major>.<minor>.<patch>`, or for an explicit on-demand run (`web` source via **CI/CD -> Pipelines -> Run pipeline**, or `api` source via `glab ci run` / the REST API), and ends with `when: never`. Ordinary pushes and merge requests now create no pipeline at all, rather than creating one that runs and bills minutes. The `verify` job's own `rules:` (merge request events, default branch) were removed as redundant — the workflow block already decides whether a pipeline exists, and leaving the old job rules in place would have matched neither a tag nor a manual run, producing a pipeline with no jobs. `.githooks/pre-push` is now unambiguously the gate for everyday work. **Consequence:** the README pipeline and coverage badges track the last pipeline that ran on `master`, so they now reflect the last deliberate run rather than the current tip.
